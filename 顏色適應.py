# -*- coding: utf-8 -*-
"""
Created on Wed Nov  3 13:27:21 2021
...
save_dir = 'C:/Users/yuwei/Desktop/work/python/color_match/datasets/test/'

root_v_defect_dom = 'C:/Users/yuwei/Desktop/work/python/color_match/datasets/free_dom/' # 參考(虛擬-貼回-模糊)
root_r_free_dom = 'C:/Users/yuwei/Desktop/work/python/color_match/datasets/defect_dom/' # 虛擬(由GAN產生)
path = os.listdir(root_v_defect_dom)

# color match loop
for img_path in path:
    print(img_path)
    # Load left and right images
    R = cv2.imread(root_r_free_dom+img_path) # 改善前
    
    ref_w = R.shape[0]
    ref_h = R.shape[1]
    
    R = cv2.resize(R, (ref_h, ref_w))
    
    R = cv2.cvtColor(R, cv2.COLOR_BGR2RGB)
    L = imread(root_v_defect_dom+img_path)# defect
    L = cv2.resize(L, (ref_h, ref_w))
    
    r, g, b = cv2.split(L)
    merge = cv2.merge([r, g, b]) # ref
    merge = cv2.GaussianBlur(merge, (5, 5), 0)

    
    # Match using the right side as reference
    matched = match_histograms(R, merge, multichannel=True) # 改善後
    
    
    # Place side-by-side and save
    result = np.hstack((R, merge, matched)) # 虛擬(由GAN產生) vs. 虛擬(貼回後模糊) vs. 直方圖均衡化調整
    imsave(save_dir+'/compare/'+img_path,result)
    imsave(save_dir+'/color_match_result/'+img_path,matched)
    
    
#%%
# way2(HSV)
# from color_matcher import ColorMatcher
# from color_matcher.io_handler import save_img_file, FILE_EXTS
# from color_matcher.normalizer import Normalizer
# from skimage.io import imread, imsave
# from skimage.color import rgb2hsv, hsv2rgb
# import os

# save_dir = 'C:/Users/yuwei/Desktop/color_match_git_hsv/'
# root_v_defect_dom = 'C:/Users/yuwei/Desktop/defect_dom/'
# root_r_free_dom = 'C:/Users/yuwei/Desktop/free_dom/'
# path = os.listdir(root_r_free_dom)
...
#     L = imread(root_v_defect_dom+img_path)# defect
#     L_hsv = rgb2hsv(L)
#     R = imread(root_r_free_dom+img_path)# free
#     R_hsv = rgb2hsv(R)
    
#     img_res = cm.transfer(src=L_hsv, ref=R_hsv, method='mkl')
#     img_res_rgb = hsv2rgb(img_res)
#     img_res = Normalizer(img_res).uint8_norm()
    
#     imsave(save_dir + img_path, img_res_rgb)
...
def color_transfer_new(sc, dc):
    ...
    img_n=(((sc-s_mean)*(t_std/s_std))+t_mean).astype('uint8')
    # return img_n
...
save_dir = 'C:/Users/yuwei/Desktop/work/python/color_match/datasets/exp3/'
root_sc = 'C:/Users/yuwei/Desktop/work/python/color_match/datasets/Defect_blur/'
root_dc = 'C:/Users/yuwei/Desktop/work/python/color_match/datasets/defect_dom/'
path = os.listdir(root_sc)

for temp in path:
    sc = cv2.imread(root_sc+temp)
    dc = cv2.imread(root_dc+temp)
    
    ouput = color_transfer_new(dc, sc)
    cv2.imwrite(save_dir+temp, ouput)
...
#     img = cv2.imread("C:/Users/yuwei/Desktop/defect_dom/img_20180710_122723_009_00_1.png")
